...
enum INSTRUCTION_TYPE {
	...
	// BX PC
	BX_THUMB16,
	// ADD <Rdn>, PC (Rd != PC, Rn != PC) 在对ADD进行修正时，采用了替换PC为Rr的方法，当Rd也为PC时，
	//由于之前更改了Rr的值，可能会影响跳转后的正常功能。
	ADD_THUMB16,
	// MOV Rd, PC
	MOV_THUMB16,
	// ADR Rd, <label>
	...
	// BX PC
	BX_ARM,
	// ADD Rd, PC, Rm (Rd != PC, Rm != PC) 在对ADD进行修正时，采用了替换PC为Rr的方法，
	//当Rd也为PC时，由于之前更改了Rr的值，可能会影响跳转后的正常功能;实际汇编中没有发现Rm也为PC的情况，故未做处理。
	ADD_ARM,
	// ADR Rd, <label>
	ADR1_ARM,
	// ADR Rd, <label>
...
static int relocateInstructionInThumb32(uint32_t pc, uint16_t high_instruction, 
uint16_t low_instruction, uint16_t *trampoline_instructions)
{
...
static void relocateInstructionInThumb(uint32_t target_addr, uint16_t *orig_instructions,
 int length, uint16_t *trampoline_instructions, int *orig_boundaries, 
 int *trampoline_boundaries, int *count)
{
	...
	while (1) {
		...
		if ((orig_instructions[orig_pos] >> 11) >= 0x1D && (orig_instructions[orig_pos] >> 11) <= 0x1F) {
			...
			orig_instructions[orig_pos + 1], &trampoline_instructions[trampoline_pos]);
			pc += sizeof(uint32_t);
...
static void relocateInstructionInArm(uint32_t target_addr, uint32_t *orig_instructions, 
int length, uint32_t *trampoline_instructions, int *orig_boundaries, 
int *trampoline_boundaries, int *count)
{
	uint32_t pc;
	uint32_t lr;
...
void *trampoline_instructions, int *orig_boundaries, int *trampoline_boundaries, int *count)
{
	if (target_addr & 1 == 1) {
		relocateInstructionInThumb(target_addr - 1, (uint16_t *) orig_instructions, 
		length, (uint16_t *) trampoline_instructions, orig_boundaries, trampoline_boundaries, count);
	}
	else {
		relocateInstructionInArm(target_addr, (uint32_t *) orig_instructions, 
		length, (uint32_t *) trampoline_instructions, orig_boundaries, trampoline_boundaries, count);
	}
}
